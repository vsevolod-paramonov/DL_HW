...
class SGD(Optimizer):
    ...
    def step(self):
        ...
        for param, grad, m in zip(parameters, gradients, self.state['m']):
          ...
          grad_new = grad + self.weight_decay * param

          np.add(self.momentum * m, grad_new, out = m)
...
class Adam(Optimizer):
    ...
    def step(self):
        ...
        for param, grad, m, v in zip(parameters, gradients, self.state['m'], self.state['v']):
          ...
          np.add(self.beta1 * m, (1 - self.beta1) * grad_new, out = m)
          np.add(self.beta2 * v, (1 - self.beta2) * (grad_new ** 2), out = v)

          hat_m = m / (1 - self.beta1 ** t)
          hat_v = v / (1 - self.beta2 ** t)

          np.subtract(param, self.lr * hat_m / (np.sqrt(hat_v) + self.eps) , out = param)
